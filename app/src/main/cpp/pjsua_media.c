...
pj_status_t pjsua_media_subsys_init(const media_cofg *cfg) {
     ...
     * 1. 创建 pjmedia_endpt 实例 endpt
     * 2. 初始化 codec manager endpt->codec
     * 3. 初始化 ioqueue 实例 endpt->ioqueue
     * 4. 创建工作线程pj_ioqueue_poll(endpt->ioqueue) 获取队列中的事件
     * */
...
/**
 * 创建 RTP 和 RTCP socket对，也可能通过 STUN 解决公共地址
 */
static pj_status_t create_rtp_rtcp_sock(pjmedia_sock_info *skinfo) {
    ...
    int i;
    pj_bool_t use_ipv6, use_nat64;      //配置，是否使用 ipv6
    int af;
    pj_sockaddr bound_addr;
    pj_sockaddr mapped_addr[2];
    pj_status_t status = PJ_SUCCESS;
    char addr_buf[PJ_INET6_ADDRSTRLEN + 10];   //56
    pj_sock_t sock[2];
