...
#if PJMEDIA_VIDEO_DEV_HAS_HYTERA
...
PJ_DECL(pj_status_t) pjmedia_vid_dev_subsys_init(pj_pool_factory *pf, pj_bool_t hytera_socket)
{
    unsigned i;
...
#if PJMEDIA_VIDEO_DEV_HAS_HYTERA
    if (hytera_socket == PJ_TRUE) {
        vid_subsys->drv[vid_subsys->drv_cnt++].create = &pjmedia_hytera_factory;
    }
#endif
