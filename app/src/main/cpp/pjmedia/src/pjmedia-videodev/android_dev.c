 ...
 */
#define MAINTAIN_ASPECT_RATIO 	PJ_FALSE
...
typedef struct and_stream
{
    ...
    pjmedia_rect_size  init_size;     // pjsip 初始化设置的视频宽高
...
static jobject pjCameraObject = NULL; /**< PjCamera instance */
...
pj_status_t pjsua_restart_camera_device() {

	JNIEnv *jni_env;
	pj_bool_t with_attach;
	with_attach = jni_get_env(&jni_env);

	if (pjCameraObject != NULL) {

		(*jni_env)->CallVoidMethod(jni_env, pjCameraObject, jobjs.cam.m_stop);

		/**
		 * 在通话中，断网，stop camera 后，需要一个延时，才能正常 start camera
		 */
//		usleep(250 * 1000);

		jint res = (*jni_env)->CallIntMethod(jni_env, pjCameraObject, jobjs.cam.m_start);
		if (res < 0) {
#if 0
            jni_detach_env(with_attach);
            PJ_LOG(3, (THIS_FILE, "failed to restart camera (err=%d)", res));
            return PJMEDIA_EVID_SYSERR;
#else
			res = (*jni_env)->CallIntMethod(jni_env, pjCameraObject, jobjs.cam.m_start);
			if (res < 0) {
		...
		pjsua_media_set_update_surfaceHolder_IOException_enable(PJ_FALSE);
		PJ_LOG(5, (THIS_FILE, "restart camera device success"));
	} else {

		PJ_LOG(3, (THIS_FILE, "restart camera fail pjCameraObject is null"));
        jni_detach_env(with_attach);
					...
					pjmedia_vid_dev_stream **p_vid_strm)
{
    ...
    strm->init_size.w = vfd->size.w;
    strm->init_size.h = vfd->size.h;
...
static pj_status_t and_stream_start(pjmedia_vid_dev_stream *s)
{
    ...
    pj_bool_t should_start = PJ_TRUE;
    ...
    if (IOException_enable == PJ_FALSE) {
        ...
        if (mediaRecStatus != PJSUA_MEDIA_REC_STATUS_RECORDING && enable != PJ_TRUE) {
            ...
            should_start = PJ_FALSE;
        }
    }

    if (should_start == PJ_TRUE) {
...
static pj_status_t and_stream_stop(pjmedia_vid_dev_stream *s)
{
    ...
    if (IOException_enable == PJ_FALSE) {
        ...
        pj_bool_t enable = pjsua_media_get_previewing_enable();
        if (mediaRecStatus == PJSUA_MEDIA_REC_STATUS_RECORDING || enable == PJ_TRUE) {
            should_stop = PJ_FALSE;
        }
    }

    if (should_stop == PJ_TRUE) {
			       ...
			       jlong user_data)
{
	...
	pjsua_take_own_screenshot((char *)f.buf, strm->init_size.w, strm->init_size.h);
...
int yuv420p_to_jpeg(pjmedia_frame *frame, int image_width,int image_height, int quality);

void i420_scale_save_to_jpg(pjmedia_frame *frame, int width, int height);
int i420_to_jpeg(char *i420_data, int image_width,int image_height, int quality);
static void I420_scale(char *src_i420_data, char *dst_i420_data, int width, int height, int dst_width, int dst_height);
...
void i420_scale_save_to_jpg(pjmedia_frame *frame, int width, int height) {
    ...
    save_count ++;
    PJ_LOG(5, (THIS_FILE, "test...save_count: %d", save_count));
    if (save_count != 100) {
        return;
    }

    char *i420Data = (char *)malloc(frame->size);
    if (i420Data == NULL) {
        return;
    }

    PJ_LOG(5, (THIS_FILE, "test...frame->size: %d", frame->size));
    pj_memcpy(i420Data, frame->buf, frame->size);

    int dstWidth = width/2;
    int dstHeight = height/2;
    char dstI420Data[dstWidth*dstHeight*3/2];

    I420_scale(i420Data, dstI420Data, width, height, dstWidth, dstHeight);

    i420_to_jpeg(dstI420Data, dstWidth, dstHeight, 100);
...
static void I420_scale(char *src_i420_data, char *dst_i420_data, int width, int height, int dst_width, int dst_height) {
    ...
    int src_ysize = width * height;
    char *src_ybase = src_i420_data;
    char *src_ubase = src_ybase + src_ysize;
    char *src_vbase = src_ybase + src_ysize * 5/4;

    int dst_ysize = dst_width *dst_height;
    char *dst_ybase = dst_i420_data;
    char *dst_ubase = dst_i420_data + dst_ysize;
    char *dst_vbase = dst_i420_data + dst_ysize * 5/4;

    I420Scale((const uint8 *)src_ybase, width,
              (const uint8* )src_ubase, width >> 1,
              (const uint8 *)src_vbase, width >> 1,
              width, height,
              (uint8 *)dst_ybase, dst_width,
              (uint8 *)dst_ubase, dst_width >> 1,
              (uint8 *)dst_vbase, dst_width >> 1,
...
int i420_to_jpeg(char *i420_data, int image_width,int image_height, int quality) {
    ...
    char *filename = "/sdcard/a_test/lfldelin_scale.jpg";
    ...
    ybase = i420_data;
    ubase = i420_data + image_width * image_height;
    vbase = i420_data + (image_width * image_height * 5/4);
    int j=0;
...
//    }
    ...
    char *pdata = (char *)malloc(frame->size);
    if (pdata == NULL) {
        return 0 ;
    }

    PJ_LOG(5, (THIS_FILE, "test...frame->size: %d, srcWidth: %d, image_height: %d", frame->size, image_width, image_height));
    pj_memcpy(pdata, frame->buf, frame->size);
    ...
    char *filename = "/sdcard/a_test/lfldelin_portrait__.jpg";
    ...
    vbase = pdata + (image_width * image_height * 5/4);
    int j=0;
    while (cinfo.next_scanline < cinfo.image_height)
    {
        int idx=0;
