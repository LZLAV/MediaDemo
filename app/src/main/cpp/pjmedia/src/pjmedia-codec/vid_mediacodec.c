...
static pj_status_t mediacodec_decodec_open(mediacodec_codec_data 	* mediacodec_data)
{
    ...
    AMediaFormat_setInt32(defmt, AMEDIAFORMAT_KEY_WIDTH, mediacodec_data->prm->dec_fmt.det.vid.size.w);
    AMediaFormat_setInt32(defmt, AMEDIAFORMAT_KEY_HEIGHT, mediacodec_data->prm->dec_fmt.det.vid.size.h);

    pj_status_t mediastatus = AMediaCodec_configure(mediacodec_data->dec, defmt, NULL, NULL, 0);
    if (mediastatus != 0) {
        PJ_LOG(4,(THIS_FILE, "decode : AMediaCodec_configure() failed with error %i ", (int)mediastatus));
    }
    else{
                                    ...
                                    pjmedia_vid_codec_param *codec_param )
{
    ...
    if (mediastatus != 0) {
        PJ_LOG(4,(THIS_FILE, "encode : AMediaCodec_configure() failed with error %i for format %u", (int)status, 21));
    }
    else
    {
    ...
    PJ_LOG(4,(THIS_FILE, "decode : AMediaFormat:MIME[%s],W[%d],H[%d]", "video/avc", param->dec_fmt.det.vid.size.w, param->dec_fmt.det.vid.size.h));
    ...
    if (!mediacodec_data->whole) {
        ...
        mediacodec_data->dec_buf = (pj_uint8_t*)pj_pool_alloc(mediacodec_data->pool, mediacodec_data->dec_buf_size);
    }
...
static pj_status_t mediacodec_codec_encode_whole(pjmedia_vid_codec *codec,
    ...
    if (ibufidx >= 0){
        ...
        if (buf) {
            ...
            PJ_LOG(4,(THIS_FILE, "encode : AMediaCodec_queueInputBuffer: ibufidx[%d] bufsize[%d] curTime[%lld]", ibufidx, ibufsize));
    ...
    if ((obufidx = AMediaCodec_dequeueOutputBuffer(mediacodec_data->enc, &info, TIMEOUT_US)) >= 0){
        PJ_LOG(4,(THIS_FILE, "encode : AMediaCodec_dequeueOutputBuffer: obufidx=%d, info.size=%d, info.flags=%d, info.offset=%d, info.presentationTimeUs=%lld ",
        ...
        if (oBuf){
            PJ_LOG(4,(THIS_FILE, "encode : AMediaCodec_getOutputBuffer: obufidx=%d, bufsize=%d,info.size=%d output_buf_len=%d",obufidx, obufsize, info.size, output_buf_len));
        ...
        if (obufidx == AMEDIA_ERROR_UNKNOWN){
            PJ_LOG(4,(THIS_FILE, "encode : AMediaCodec_dequeueOutputBuffer() had an exception, MediaCodec is lost"));
        }
    }
    ...
    *has_more = PJ_FALSE;
    ...
    if (mediacodec_data->whole) {
        ...
        if(mediacodec_data->enc_buf_is_keyframe)
                //if(false)
        {
            memmove(mediacodec_data->enc_buf+mediacodec_data->setslen, mediacodec_data->enc_buf, mediacodec_data->enc_frame_size);
        ...
        status = pjmedia_h264_packetize(mediacodec_data->pktz, (pj_uint8_t*)mediacodec_data->enc_buf,
                                        mediacodec_data->enc_frame_size, &mediacodec_data->enc_processed,
                                        &payload, &payload_len);
        if (status != PJ_SUCCESS)
            return status;
                                             ...
                                             pjmedia_frame *output)
{
    ...
    if (input)
    {
        ...
        if (iBufidx >= 0)
        {
            ...
            AMediaCodec_queueInputBuffer(mediacodec_data->dec, iBufidx, 0, ibufsize, timeGetTime(), 0);
        }
        else if (iBufidx == -1)
        {
            /*
        ...
        if (oBufidx >= 0)
        {
            ...
            PJ_LOG(4,(THIS_FILE, "decode : info: offset[%d] flags[%d] size[%d] presentationTimeUs[%ld]",
            ...
            else
            {
                ...
                if (width != 0 && height != 0)
                {
                    PJ_LOG(4,(THIS_FILE, "decode : buf[%d]info.size[%d]size[%dx%d]color-format[%d]",
            ...
            PJ_LOG(4,(THIS_FILE, "decode : AMediaCodec_dequeueOutputBuffer() oBufidx=%d", oBufidx));
        }
