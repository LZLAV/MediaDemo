...
static pj_status_t h264_preopen(ffmpeg_private *ff)
{
	...
	/* Apply intra-refresh */
	if (!AV_OPT_SET_INT(ctx->priv_data, "intra-refresh", 10))
	{
                                     ...
                                     pj_mutex_t *ff_mutex)
{
    ...
    if (!AV_OPT_SET_INT(ff->enc_ctx->priv_data, "rc-lookahead", 0)) {
        PJ_LOG(3, (THIS_FILE, "Failed to set x264 rc-lookahead 0"));
    }

    ff->enc_ctx->max_b_frames = 0;
    ff->enc_ctx->has_b_frames = 0;

    // other
//    ff->enc_ctx->gop_size = 2;

    pj_bool_t enable_status = pjsua_media_multi_enable_status(PJSUA_MULTI_TYPE_VID_ENCODE);
    if (enable_status == PJ_TRUE) {

        pjmedia_rect_size rect_size = pjsua_media_multi_get_rect_size(
                PJSUA_MULTI_TYPE_VID_ENCODE);

        ff->enc_ctx->width = rect_size.w;
        ff->enc_ctx->height = rect_size.h;

        int fps = vfd->fps.num;
        ff->enc_ctx->bit_rate = ff->enc_ctx->width * ff->enc_ctx->height * fps / 8;
    }
				      ...
				      pjmedia_vid_codec_param *attr )
{
    ...
    if (enable_status == PJ_TRUE) {
        ...
        ff->enc_vafp.size = pjsua_media_multi_get_rect_size(
                PJSUA_MULTI_TYPE_VID_ENCODE);
    }
