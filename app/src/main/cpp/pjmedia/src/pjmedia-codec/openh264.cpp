                                    ...
                                    pjmedia_vid_codec_param *codec_param )
{
    ...
    if (enable_status == PJ_TRUE) {
        ...
        pjmedia_rect_size rect_size = pjsua_media_multi_get_rect_size(PJSUA_MULTI_TYPE_VID_ENCODE);
        eprm.iPicWidth = rect_size.w;
        eprm.iPicHeight = rect_size.h;

        int fps = param->enc_fmt.det.vid.fps.num;
        long bps = rect_size.w * rect_size.h * fps / 8;
        eprm.iTargetBitrate = bps;
    }
                                           ...
                                           pj_bool_t *has_more)
{
    ...
    // 编出来的是未加工的 h264 数据
//    pjmedia_frame frame;
//    frame.buf = oh264_data->enc_frame_whole;
//    frame.size = oh264_data->enc_frame_size;

//    pjsua_pre_vid_put_frame(NULL, &frame);
