...
extern int MIC_SOURCE;
extern int STREAM_SOURCE;
...
static int AndroidRecorderCallback(void *userData)
{
    ...
    long read_method_time = 0, byte_array_time1 = 0, rec_cb_time = 0, byte_array_time2 = 0;
    int count = 0;
    ...
    while (!stream->quit_flag) {
        ...
        pj_time_val lasttime,now;
        pj_gettimeofday(&lasttime);
        ...
        pj_gettimeofday(&now);
        read_method_time += PJ_TIME_VAL_MSEC(now) - PJ_TIME_VAL_MSEC(lasttime);
        lasttime = now;
        ...
        pj_gettimeofday(&now);
        byte_array_time1 += PJ_TIME_VAL_MSEC(now) - PJ_TIME_VAL_MSEC(lasttime);
        lasttime = now;

        status = (*stream->rec_cb)(stream->user_data, &frame);

        pj_gettimeofday(&now);
        rec_cb_time += PJ_TIME_VAL_MSEC(now) - PJ_TIME_VAL_MSEC(lasttime);
        lasttime = now;
        ...
        pj_gettimeofday(&now);
        byte_array_time2 += PJ_TIME_VAL_MSEC(now) - PJ_TIME_VAL_MSEC(lasttime);
        lasttime = now;
...
//        if(get_vid_record_state() == SDK_BELOW_19) {
//            aac_encode(getaac_instance(), frame.buf, frame.size,p_out_buf,&p_out_size);
//        }
...
//                vid_save_add_aud_data(frame.buf,size);
//            }
//            else if(get_vid_record_state() == SDK_BELOW_19){
//                if(p_out_size > 0){
//                    vid_save_add_aud_data(p_out_buf,p_out_size);
//                }
//            }
        }
...
//        }
    ...
    if (stream->dir & PJMEDIA_DIR_PLAYBACK) {
        ...
        track_obj = (*jni_env)->NewObject(jni_env,
                                          stream->track_class,
                                          constructor_method,
                                          STREAM_SOURCE, /* STREAM_VOICE_CALL */
                                          param->clock_rate,
                                          channelOutCfg,
                                          sampleFormat,
                                          inputBuffSizePlay,
