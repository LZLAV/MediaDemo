...
struct pjmedia_vid_stream
{
	...
	pjmedia_frame           *scale_frm_buf; /**< libyuv scale_I420 frame_buf*/
	pj_size_t               scale_frm_buf_size;
	pjmedia_rect_size	    preview_size;	/**< preview Video size (width, height) 	*/
...
/* Prototypes */
static pj_status_t rx_decode_frame(pjmedia_vid_stream *stream,
								pjmedia_frame *frame);

static pj_status_t enc_decode_frame(pjmedia_vid_stream *stream,
                                   pjmedia_frame *frame);
							 ...
							 pj_bool_t with_bye)
{
	...
	if(!with_bye)
	{
		...
		if(status != PJ_SUCCESS)
		{
			PJ_PERROR(4,(stream->name.ptr,status,"Error generation RTCP Pli"));
		} else{
			len += (int)pli_len;
		}
	}
...
static void on_rx_rtp( pjmedia_tp_cb_param *param)
{
	...
	if ((pj_ntohl(hdr->ts) != stream->dec_frame.timestamp.u32.lo) || hdr->m) {
		if (PJMEDIA_VID_STREAM_SKIP_PACKETS_TO_REDUCE_LATENCY) {
			...
			if (rx_decode_frame(stream, &stream->dec_frame) != PJ_SUCCESS) {
							 ...
							 pjmedia_frame *frame)
{
	...
	pjmedia_frame *encode_frame = frame;
#if TRACE_JB
	pj_bool_t enable_status = pjsua_media_multi_enable_status(PJSUA_MULTI_TYPE_VID_ENCODE);
	if (enable_status == PJ_TRUE) {

		pjmedia_rect_size src_size = stream->preview_size;
		pjmedia_rect_size dst_size = pjsua_media_multi_get_rect_size(PJSUA_MULTI_TYPE_VID_ENCODE);

		pj_size_t previewSize = (src_size.w * src_size.h * 3/2);
		if (previewSize >= frame->size) {

			pjsua_vid_size_change(frame, &src_size, stream->scale_frm_buf, &dst_size);
			encode_frame = stream->scale_frm_buf;
		} else {
			PJ_LOG(5, (THIS_FILE, "frame.size: %d. preview.size: %d, is error", frame->size, previewSize));
		}
	}
#endif

	/* Encode! */
	status = pjmedia_vid_codec_encode_begin(stream->codec, &enc_opt, encode_frame,
											channel->buf_size -
							 ...
							 pjmedia_frame *frame)
{
	...
	if (stream->dec_frame.size == 0) {
		...
		if (enc_decode_frame(stream, frame) != PJ_SUCCESS) {
			frame->type = PJMEDIA_FRAME_TYPE_NONE;
			frame->size = 0;
		}
	} else {
		...
		pjmedia_vid_stream **p_stream)
{
	...
	stream->preview_size.w = vfd_enc->size.w;
	stream->preview_size.h = vfd_enc->size.h;
    pj_size_t preview_frame_size = vfd_enc->size.w * vfd_enc->size.h * 3 / 2;
	PJ_LOG(5, (THIS_FILE, "preview_size.w: %d, preview_size.h: %d, preview_frame_size: %d.",
			vfd_enc->size.w, vfd_enc->size.h, preview_frame_size));
