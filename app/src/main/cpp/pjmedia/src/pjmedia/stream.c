...
//#include <pjmedia/src/pjmedia-audiodev/aac_enc.h>
...
static char trace_jb_path_prefix[200] = "";
#if TRACE_JB
...
{
    if (pjsua_var.media_cfg.jitter_buffer_enable_type == 1) {
        pj_time_val now;
...
static pj_status_t get_frame(pjmedia_port *port, pjmedia_frame *frame) {
    ...
    for (samples_count = 0; samples_count < samples_required;) {
        ...
        if (frame_type == PJMEDIA_JB_MISSING_FRAME) {
            ...
            pj_sound_receive_record_decodec_before(&frame_in, samples_per_frame);
...
static pj_status_t put_frame_imp(pjmedia_port *port,
    ...
    if (stream->tx_dtmf_count) {
        ...
        pj_sound_mic_record_send_before(&frame_out);
        /* Encapsulate. */
        status = pjmedia_rtp_encode_rtp(&channel->rtp,
                                        channel->pt, 0,
                                        (int) frame_out.size, rtp_ts_len,
...
#define DEFAULT_MAX_FILE_PATH_LEN 300
PJ_DEF(pj_status_t)

pjmedia_stream_check_dir_folder_exist(void) {

    pj_status_t status = PJ_SUCCESS;
    pj_uint16_t pathLen = strlen(trace_jb_path_prefix);
    if (pathLen > DEFAULT_MAX_FILE_PATH_LEN) {

        PJ_LOG(3,
               (THIS_FILE, "Error, The directory name lenght should not be greater than %d chars", DEFAULT_MAX_FILE_PATH_LEN));
        return PJ_EINVAL;
    }

    char str[DEFAULT_MAX_FILE_PATH_LEN];
    strncpy(str, trace_jb_path_prefix, DEFAULT_MAX_FILE_PATH_LEN);
    pathLen = strlen(str);
    ...
    if (status != PJ_SUCCESS) {
        ...
        PJ_LOG(3, (THIS_FILE, "Error, mkdir failed,  May not have permission"));
        return PJ_EINVAL;
    }
...
pjmedia_stream_jitter_buffer_store_path_config(void) {

    char logDirPath[150] = "";
    char *rootDir = "/sdcard/audio_store/";
#if TRACE_JB
    if (pjsua_var.log_cfg.log_filename.slen > 0) {

        int name_len = pjsua_var.log_cfg.log_filename.slen;
        char *file_name = pjsua_var.log_cfg.log_filename.ptr;

        int end_index = 0;
        for (end_index = (name_len - 1); end_index >= 0; end_index--) {
        ...
        for (int index = 0; index < end_index; ++index) {
            logDirPath[index] = file_name[index];
        }
    ...
    size_t rootDirPathSize = strlen(rootDir);

    char jitterBufferDir[] = "debugJitterBuffer";

    int dirPathLen = rootDirPathSize + strlen(jitterBufferDir) + 3;
    pj_bzero(trace_jb_path_prefix, dirPathLen);

    sprintf(trace_jb_path_prefix, "%s/%s/", rootDir, jitterBufferDir);

    PJ_LOG(5, (THIS_FILE, "trace_jb_path_prefix: %s.", trace_jb_path_prefix));
    pjmedia_stream_check_dir_folder_exist();
    return PJ_SUCCESS;
}
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_create( pjmedia_endpt
*endpt,
pj_pool_t *pool,
...
PJ_ASSERT_RETURN(endpt
&& info && p_stream, PJ_EINVAL);

if (pool == NULL) {
own_pool = pjmedia_endpt_create_pool(endpt, "strm%p",
                                     PJMEDIA_STREAM_SIZE,
                                     PJMEDIA_STREAM_INC);
PJ_ASSERT_RETURN(own_pool
!= NULL, PJ_ENOMEM);
pool = own_pool;
}
...
pj_strdup(pool,
&stream->si.fmt.encoding_name, &info->fmt.encoding_name);
if (info->param)
stream->si.
param = pjmedia_codec_param_clone(pool, info->param);
pjmedia_rtcp_fb_info_dup(pool,
&stream->si.loc_rtcp_fb,
&info->loc_rtcp_fb);
pjmedia_rtcp_fb_info_dup(pool,
&stream->si.rem_rtcp_fb,
&info->rem_rtcp_fb);

/* Init stream/port name */
name.
ptr = (char *) pj_pool_alloc(pool, M);
name.
slen = pj_ansi_snprintf(name.ptr, M, "strm%p", stream);
...
afd->
channel_count = info->fmt.channel_cnt;
stream->port.port_data.
pdata = stream;
...
user_data = user_data;
stream->
rtcp_interval = (PJMEDIA_RTCP_INTERVAL - 500 + (pj_rand() % 1000)) *
                info->fmt.clock_rate / 1000;
stream->
rtcp_sdes_bye_disabled = info->rtcp_sdes_bye_disabled;

stream->
tx_event_pt = info->tx_event_pt ? info->tx_event_pt : -1;
stream->
rx_event_pt = info->rx_event_pt ? info->rx_event_pt : -1;
stream->
last_dtmf = -1;
stream->
...
stream->cname.
ptr = p = (char *) pj_pool_alloc(pool, 20);
pj_create_random_string(p,
5);
p += 5;
*p++ = '@'; *p++ = 'p'; *p++ = 'j';
pj_create_random_string(p,
...
stream->codec_param.info.
max_bps = stream->codec_param.info.avg_bps;

/* Check for invalid frame per packet. */
if (stream->codec_param.setting.frm_per_pkt < 1)
stream->codec_param.setting.
frm_per_pkt = 1;
...
clock_rate = info->fmt.clock_rate;
stream->codec_param.info.
channel_cnt = info->fmt.channel_cnt;

/* Allocate decoding buffer as Opus can send a packet duration of
 * up to 120 ms.
 */
stream->
dec_buf_size = stream->codec_param.info.clock_rate * 120 /
               1000;
stream->
dec_buf = (pj_int16_t *) pj_pool_alloc(pool,
                                       stream->dec_buf_size *
                                       sizeof(pj_int16_t));
}
...
stream->
dec_ptime = stream->codec_param.info.frm_ptime;
afd->
bits_per_sample = 16;
afd->
frame_time_usec = stream->codec_param.info.frm_ptime *
                  stream->codec_param.setting.frm_per_pkt * 1000;
stream->port.info.fmt.
id = stream->codec_param.info.fmt_id;
if (stream->codec_param.info.fmt_id == PJMEDIA_FORMAT_L16) {
/* Raw format */
afd->
avg_bps = afd->max_bps = afd->clock_rate * afd->channel_count *
                         afd->bits_per_sample;
...
/* Encoded format */
afd->
avg_bps = stream->codec_param.info.avg_bps;
afd->
max_bps = stream->codec_param.info.max_bps;
...
stream->
enc_samples_per_pkt = stream->codec_param.info.enc_ptime *
                      stream->codec_param.info.channel_cnt *
                      afd->clock_rate / 1000;
...
/* Allocate buffer */
stream->
enc_buf_size = afd->clock_rate * ptime / 1000;
stream->
enc_buf = (pj_int16_t *)
...
stream->
enc_samples_per_pkt = PJMEDIA_AFD_SPF(afd);
}


/* Initially disable the VAD in the stream, to help traverse NAT better */
stream->
vad_enabled = stream->codec_param.setting.vad;
if (PJMEDIA_STREAM_VAD_SUSPEND_MSEC > 0 && stream->vad_enabled) {
stream->codec_param.setting.
vad = 0;
stream->
ts_vad_disabled = 0;
pjmedia_codec_modify(stream
->codec, &stream->codec_param);
...
stream->
frame_size = stream->codec_param.info.max_rx_frame_size;
} else {
stream->
frame_size = stream->codec_param.info.max_bps *
             stream->codec_param.info.frm_ptime / 8 / 1000;
if ((stream->codec_param.info.
max_bps *
        stream
->codec_param.info.frm_ptime) % 8000 != 0)
{
++stream->
frame_size;
}
...
stream->
max_plc_cnt = (MAX_PLC_MSEC + stream->codec_param.info.frm_ptime - 1) /
              stream->codec_param.info.frm_ptime;
/* Disable PLC until a "NORMAL" frame is gotten from the jitter buffer. */
stream->
plc_cnt = stream->max_plc_cnt;
...
if (0)//maojh 2021.03.17
{
/* Set up jitter buffer */
pjmedia_jbuf_set_adaptive(stream
->jb, jb_init, jb_min_pre, jb_max_pre);
}
else//for debug or for test //maojh 2021.03.17
{
pjmedia_jbuf_set_fixed(stream
->jb, jb_init);
}
...
rtcp_setting.
name = stream->port.info.name.ptr;
rtcp_setting.
ssrc = info->ssrc;
rtcp_setting.
rtp_ts_base = pj_ntohl(stream->enc->rtp.out_hdr.ts);
rtcp_setting.
clock_rate = info->fmt.clock_rate;
rtcp_setting.
samples_per_frame = PJMEDIA_AFD_SPF(afd);
...
stream->rtcp.stat.
rtp_tx_last_seq = info->rtp_seq;
stream->rtcp.stat.
rtp_tx_last_ts = info->rtp_ts;
}

/* Subscribe to RTCP events */
pjmedia_event_subscribe(NULL,
&stream_event_cb, stream,
&stream->rtcp);
}
 ...
 */
stream->
out_rtcp_pkt_size = sizeof(pjmedia_rtcp_sr_pkt) +
...
stream->
out_rtcp_pkt_size = PJMEDIA_MAX_MTU;

stream->
out_rtcp_pkt = pj_pool_alloc(pool, stream->out_rtcp_pkt_size);
...
att_param.
addr_len = pj_sockaddr_get_len(&info->rem_addr);
att_param.
rtp_cb2 = &on_rx_rtp;
att_param.
rtcp_cb = &on_rx_rtcp;
...
#if TRACE_JB
if (pjsua_var.media_cfg.jitter_buffer_enable_type == 1)//maojh 2021.03.10  JB
{
char trace_name[PJ_MAXPATH];
pj_ssize_t len;

pjmedia_stream_jitter_buffer_store_path_config();

pj_ansi_snprintf(trace_name,
sizeof(trace_name),
"%s" "%s.csv",
trace_jb_path_prefix, stream->port.info.name.ptr);

status = pj_file_open(pool, trace_name, PJ_O_WRONLY,
                      &stream->trace_jb_fd);
if (status != PJ_SUCCESS) {
stream->
trace_jb_fd = TRACE_JB_INVALID_FD;
PJ_PERROR(3,(THIS_FILE, status,
"Failed creating RTP trace file '%s'", trace_name));
} else {
stream->
trace_jb_buf = (char *) pj_pool_alloc(pool, PJ_LOG_MAX_SIZE);
...
pj_file_write(stream
->trace_jb_fd, stream->trace_jb_buf, &len);
pj_file_flush(stream
->trace_jb_fd);
}
}
#endif
...
frame_out.
buf = ((char *) channel->out_pkt) + sizeof(pjmedia_rtp_hdr);
frame_out.
size = 0;

create_dtmf_payload(stream,
&frame_out, 1, &first, &last);
...
/* Copy RTP header to the beginning of packet */
pj_memcpy(channel
->out_pkt, rtphdr, sizeof(pjmedia_rtp_hdr));
...
pjmedia_event_unsubscribe(NULL,
&stream_event_cb, stream,
&stream->rtcp);
 ...
 */
if (stream->transport) {
pjmedia_transport_detach(stream
->transport, stream);
stream->
transport = NULL;
}
...
if (stream->codec) {
pjmedia_codec_close(stream
->codec);
pjmedia_codec_mgr_dealloc_codec(stream
->codec_mgr, stream->codec);
stream->
codec = NULL;
}
...
if (stream->jb_mutex) {
pj_mutex_unlock(stream
->jb_mutex);
pj_mutex_destroy(stream
->jb_mutex);
stream->
jb_mutex = NULL;
}
...
if (stream->jb)
pjmedia_jbuf_destroy(stream
->jb);

#if TRACE_JB
if (TRACE_JB_OPENED(stream)) {
pj_file_close(stream
->trace_jb_fd);
stream->
trace_jb_fd = TRACE_JB_INVALID_FD;
}
...
pjmedia_stream_get_last_jb_frame_type(pjmedia_stream
*stream)
{
return stream->
jb_last_frm;
...
*)
pjmedia_stream_get_transport(pjmedia_stream
*st)
{
return st->
transport;
...
PJ_ASSERT_RETURN(stream
&& stream->enc && stream->dec, PJ_EINVALIDOP);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_reset_stat(pjmedia_stream
*stream)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_pause( pjmedia_stream
*stream,
pjmedia_dir dir
)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_resume( pjmedia_stream
*stream,
pjmedia_dir dir
)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_dial_dtmf( pjmedia_stream
*stream,
...
stream->tx_dtmf_buf[stream->tx_dtmf_count+i].
event = pt;
stream->tx_dtmf_buf[stream->tx_dtmf_count+i].
duration = 0;
stream->tx_dtmf_buf[stream->tx_dtmf_count+i].
...
stream->tx_dtmf_count += (int)digit_char->
slen;
}
 ...
 */
PJ_DEF(pj_bool_t)
pjmedia_stream_check_dtmf(pjmedia_stream
*stream)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_get_dtmf( pjmedia_stream
*stream,
char *digits,
unsigned *size
)
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_set_dtmf_callback(pjmedia_stream
*stream,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_send_rtcp_sdes( pjmedia_stream
*stream )
{
PJ_ASSERT_RETURN(stream, PJ_EINVAL
);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_send_rtcp_bye( pjmedia_stream
*stream )
{
PJ_ASSERT_RETURN(stream, PJ_EINVAL
);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_stream_get_rtp_session_info(pjmedia_stream
*stream,
pjmedia_stream_rtp_sess_info *session_info
)
{
session_info->
rx_rtp = &stream->dec->rtp;
session_info->
tx_rtp = &stream->enc->rtp;
session_info->
rtcp = &stream->rtcp;
return
PJ_SUCCESS;
