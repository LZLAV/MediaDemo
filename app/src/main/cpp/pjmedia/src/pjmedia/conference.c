...
#include <pj/pool.h>
#include <pj/string.h>
#include <pjlib-util/sound_record.h>
					       ...
					       int adj_level )
{
	...
	++dst_port->transmitter_cnt;
        ...
        PJ_LOG(4,(THIS_FILE,
        "conf->connect_cnt=%d, src_port->listener_cnt=%d, dst_port->transmitter_cnt=%d",
        conf->connect_cnt,
        src_port->listener_cnt,
        dst_port->transmitter_cnt));

        //if (conf->connect_cnt == 1)
        if(src_port->listener_cnt == 1 && src_slot == 0)
            start_sound = 1;
						  ...
						  unsigned sink_slot )
{
	...
	if(src_slot == 0 && src_port->listener_cnt == 0)
        stop_sound = PJ_TRUE;
	PJ_LOG(4,(THIS_FILE,
					      ...
					      unsigned port )
{
	...
	for (j=0; j<src_port->listener_cnt; ++j) {
		...
		--conf->connect_cnt;
		--src_port->listener_cnt;
		if(i == 0 && src_port->listener_cnt == 0)
			stop_sound = PJ_TRUE;
		break;
	    }
	}
			     ...
			     pjmedia_frame *frame)
{
	...
	pj_sound_receive_record_play_before(frame,
			                            conf->samples_per_frame);
    } else {
...
void(*resume_sound)())
{
	conf->resume_sound = resume_sound;
	return PJ_SUCCESS;
}
