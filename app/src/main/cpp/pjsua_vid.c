...
pj_status_t pjsua_vid_subsys_init(void)
{
    ...
    pj_bool_t hytera_socket = pjsua_var.media_cfg.hytera_socket;
//    if (hytera_socket == PJ_TRUE) {
//
//#if	defined(PJMEDIA_HAS_VIDEO)
//
//        status = pjmedia_codec_hytera_media_codec_init(NULL, &pjsua_var.cp.factory);
//        if (status != PJ_SUCCESS) {
//            PJ_PERROR(1,(THIS_FILE, status,
//                    "Error initializing Video Mediacodec codec"));
//            goto on_error;
//        }
//#endif
//
//    } else
...
#if    defined(PJMEDIA_HAS_VIDEO) && defined(PJMEDIA_HAS_VID_MEDIACODEC_CODEC) && PJMEDIA_HAS_VID_MEDIACODEC_CODEC != 0
        status = pjmedia_codec_vid_mediacodec_init(NULL, &pjsua_var.cp.factory);
        if (status != PJ_SUCCESS) {
        PJ_PERROR(1,(THIS_FILE, status,
                 "Error initializing Video Mediacodec codec"));
        goto on_error;
...
#if    defined(PJMEDIA_HAS_VIDEO) && defined(PJMEDIA_HAS_VID_ADAPTER_CODECS) && PJMEDIA_HAS_VID_ADAPTER_CODECS != 0
        status = pjmedia_codec_vid_adapter_codecs_init(NULL, &pjsua_var.cp.factory);
        if (status != PJ_SUCCESS) {
            PJ_PERROR(1, (THIS_FILE, status,
                    "Error initializing Video Mediacodec codec"));
            goto on_error;
...
#if    defined(PJMEDIA_HAS_VIDEO) && defined(PJMEDIA_HAS_OPENH264_CODEC) && PJMEDIA_HAS_OPENH264_CODEC != 0
    ...
    status = pjmedia_vid_dev_subsys_init(&pjsua_var.cp.factory,  hytera_socket);
    if (status != PJ_SUCCESS) {
	PJ_PERROR(1,(THIS_FILE, status,
...
//
//#if    defined(PJMEDIA_HAS_VIDEO)
//        pjmedia_codec_hytera_media_codec_deinit();
//#endif
//    } else
    	{
...
#if    defined(PJMEDIA_HAS_VIDEO) && defined(PJMEDIA_HAS_VID_MEDIACODEC_CODEC) && PJMEDIA_HAS_VID_MEDIACODEC_CODEC != 0
        pjmedia_codec_vid_mediacodec_deinit();
#endif

#if    defined(PJMEDIA_HAS_VIDEO) && defined(PJMEDIA_HAS_VID_ADAPTER_CODECS) && PJMEDIA_HAS_VID_ADAPTER_CODECS != 0
        pjmedia_codec_vid_adapter_codecs_deinit();
#endif
...
/**
 * @brief 检测是否有视频通话，如果有则获取视频通话 id
 */
pj_status_t pjsua_vid_check_video_call_processing(int *call_index) {

	pj_bool_t processing = -1;

	pjsua_call_id ids[PJSUA_MAX_CALLS];
	unsigned count = PJ_ARRAY_SIZE(ids);

	pj_status_t status = pjsua_enum_calls(ids, &count);
	if (status != PJ_SUCCESS) {
		return processing;
	}
	...
	for (int index = 0; index < count; ++index) {
		...
		pjsua_call_id callId = ids[index];
		...
		if (call_info.media_cnt >= 2) {
			...
			for (int i = 0; i < call_info.media_cnt; ++i) {
				...
				pjsua_call_media_info media = call_info.media[i];
				if (media.type == PJMEDIA_TYPE_VIDEO
					&& media.dir != PJMEDIA_DIR_NONE
					&& media.status != PJSUA_CALL_MEDIA_NONE
					&& media.status != PJSUA_CALL_MEDIA_ERROR) {
...
pj_status_t pjsua_vid_channel_init(pjsua_call_media *call_med)
{
	...
	int call_index = -1;
	status = pjsua_vid_check_video_call_processing(&call_index);
	if (status == PJ_SUCCESS) {

		pjsua_call *call = &pjsua_var.calls[call_index];

		/* Process each media stream */
		for (int mi=0; mi < call->med_cnt; ++mi) {

			pjsua_call_media *call_media = &call->media[mi];
			if (call_media->type == PJMEDIA_TYPE_VIDEO) {

				call_med->strm.v.cap_dev = call_media->strm.v.cap_dev;
				PJ_LOG(4,(THIS_FILE,
						"pjsua_vid_channel_init strm.v.cap_dev=%d",call_media->strm.v.cap_dev));
			}
		}
	}

	PJ_LOG(4, (THIS_FILE, "processing status: %d, call_index: %d, strm.v.cap_dev: %d", status, call_index, call_med->strm.v.cap_dev));
                                            ...
                                            const pjsua_vid_preview_param *prm)
{
	...
	status = pjsua_vid_codec_param_get_cur(&codec_param);
	if (status == PJ_SUCCESS) {

		unsigned width = codec_param.enc_fmt.det.vid.size.w;
		unsigned height = codec_param.enc_fmt.det.vid.size.h;
		unsigned fps_num = codec_param.enc_fmt.det.vid.fps.num;
		unsigned fps_denum = codec_param.enc_fmt.det.vid.fps.denum;

		pjmedia_format format;
		pjmedia_format_init_video(&format, PJMEDIA_FORMAT_I420, width, height, fps_num, fps_denum);
		fmt = &format;
	}
    ...
    if (!pjmedia_vid_port_is_running(w->vp_cap)) {
		...
		pj_bool_t IOException_enable = pjsua_media_get_update_surfaceHolder_IOException_enable();
		if (IOException_enable == PJ_TRUE) {

			PJ_LOG(5, (THIS_FILE, "IOException_enable is true, it will be restart camera device"));
			status = pjsua_restart_camera_device();
			if (status != PJ_SUCCESS) {
...
PJ_DEF(pj_status_t) pjsua_vid_preview_stop(pjmedia_vid_dev_index id)
{
	...
	pj_bool_t IOException_enable = pjsua_media_get_update_surfaceHolder_IOException_enable();
	if (IOException_enable == PJ_FALSE) {

		pjsua_media_rec_status mediaRecStatus = pjsua_media_own_recorder_status();
		pj_bool_t enable = pjsua_media_get_previewing_enable();
		if (mediaRecStatus == PJSUA_MEDIA_REC_STATUS_RECORDING || enable == PJ_TRUE) {
			return PJ_SUCCESS;
		}
	}
	...
	PJ_LOG(4, (THIS_FILE, "pjsua_vid_preview_stop origin id: %d", id));
    wid = pjsua_vid_preview_get_win(id);
    if (wid == PJSUA_INVALID_ID) {

		pj_uint8_t dev_count = 2;
		pj_uint8_t dev_ids[] = {1, 2};
		for (int index = 0; index < dev_count; ++index) {

			PJ_LOG(4, (THIS_FILE, "dev_id = %d, index = %d, id: %d", dev_ids[index], index, id));
			if (dev_ids[index] != id) {

				wid = pjsua_vid_preview_get_win(dev_ids[index]);
				break;
			}
		}
    ...
    PJ_LOG(4,(THIS_FILE, "Stopping preview for cap_dev=%d", wid));
    pj_log_push_indent();
...
/**
 * @brief 更新通话视频流视频设备捕获 id
 */
static void update_cap_dev(pjmedia_vid_dev_index new_cap_dev) {
	...
	for (int index = 0; index < count; ++index) {
		...
		pjsua_call_id call_id = ids[index];
		pjsua_call_info call_info;
		...
		if (call_info.media_cnt >= 2) {
			...
			pjsua_call_media_info media = call_info.media[1];
			if (media.type == PJMEDIA_TYPE_VIDEO
				&& media.dir != PJMEDIA_DIR_NONE
				&& media.status != PJSUA_CALL_MEDIA_NONE
				&& media.status != PJSUA_CALL_MEDIA_ERROR) {

				pjsua_call *call = &pjsua_var.calls[index];
				...
				if (first_active != -1) {
					...
					w->preview_cap_id = new_cap_dev;
					call_med->strm.v.cap_dev = new_cap_dev;

					PJ_LOG(4,(THIS_FILE, "update_cap_dev strm.v.cap_dev :%d",new_cap_dev));
				}
			}
		}
	}
				       ...
				       pjmedia_vid_dev_index cap_dev)
{
    ...
    if (status != PJ_SUCCESS || info.dir != PJMEDIA_DIR_CAPTURE) {
		...
		PJ_LOG(4, (THIS_FILE, "Verify the capture device failed"));
		return PJ_EINVAL;
    }
    ...
    if (call_med->strm.v.cap_dev == cap_dev) {
    	...
    	PJ_LOG(4, (THIS_FILE, "The specified capture device is being used already, cap_dev: %d, call_med->strm.v.cap_dev: %d", cap_dev, call_med->strm.v.cap_dev));
		return PJ_SUCCESS;
    }
